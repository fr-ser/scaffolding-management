...
import { neverFunction } from "@/helpers/utils";
import { type Article, type Client, type Invoice, type InvoiceDocument, type Offer, type OfferDocument, type Order, type OverdueNoticeDocument } from "@/global/types/entities";
import type { ArticleUpdate, ArticleCreate, ClientCreate, ClientUpdate, OrderCreate, OrderUpdate, OfferCreate, InvoiceUpdate } from "@/global/types/dataEditTypes";
...
export async function createDocumentBySubOrder(
  ...
  const response = await axiosInstance.post(`/api/orders/${getUrlEntityForKind(kind)}/${id}/documents/create`);
  return response.data;
}
...
export async function getDocumentsBySubOrder(
  ...
  const response = await axiosInstance.get(`/api/orders/${getUrlEntityForKind(kind)}/${subOrderId}/documents`);
  return response.data;
}
